...
try:
    ...
    log.info("Templates folder: %s", TEMPLATE_DIR)
    log.info("Templates contents: %s", files)
except Exception as e:
    log.error("Error listing templates: %s", e)
...
@app.route("/health")
def health():
    return {"status": "ok", "template_dir": TEMPLATE_DIR}, 200

@app.route("/_tmpl")
def tmpl_list():
    try:
        files = os.listdir(TEMPLATE_DIR)
        return {"templates": files}, 200
    except Exception as e:
